...
class RSIParameter(object):
    ...
    def xover(self, obj):
        ...
        if tmp < 30:
            ...
            p.bottom = self.bottom + random.choice([4, -4])
            p.top = self.top + random.choice([4, -4])
...
class RSIIndicator(BaseStrategy):
    ...
    def next(self) -> int:
        if self.rsi_A[-1] < self.params_RSI.bottom and self.rsi_A[-2] < self.params_RSI.bottom:
            # buy
            return 1
        elif self.rsi_A[-1] > self.params_RSI.top and self.rsi_A[-2] > self.params_RSI.top:
            # sell
            return -1
        else:
            return 0
    ...
    def init(self):
        self.rsi_A = self.I(RSI, self.data['Close'], self.params_RSI.period_A)
